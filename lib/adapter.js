...
// investigating create table
var fs = require('fs');
var logfile = fs.openSync('postsql-adapter.log','w');
// /investigating create table
...
/*          
          ...
          client.query("SELECT add_crud_functions($1)",  [table.replace(/['"]/g, '')], function(err, result) {
              if(err) return cb(err);
              cb();
          });
*/
    ...
    create: function(table, data, cb) {
      ...
      spawnConnection(function __CREATE__(client, cb) {
        ...
        var query = "SELECT * FROM " + table + "_create($1);\n";
        
        // Run Query
        client.query(query, [data], function __CREATE__(err, result) {
          if(err) return cb(err);

          // Cast special values
          var values = result.rows[0].data_out;

          // needs a rewrite to pass only over date-ish fields
          // perhaps by making a list of such field names at register time
          for (_field in dbs[table].definition) {
            switch(dbs[table].definition[_field].type) {
              case 'date':
              case 'datestamp':
              case 'datetime':
                if (values[_field]) {  // leave NULL fields alone
                  values[_field] = new Date(values[_field]+":00"); // works if pg datestyle == ISO
                }
            }
          }
...
//        var _query = new Query(dbs[table].definition);
        ...
        // Escape Table Name
        table = utils.escapeTable(table);
        table_clean = table.replace(/["']/g, "");

        // Build Query
        var query = "SELECT * FROM " + table_clean + "_find($1);"

        // Run Query
        client.query(query, [options], function __FIND__(err, result) {
          if(err) return cb(err);
          ...
          row = row.data_out;
          // TODO: figure out proper casting from JSON records
          //       maybe add _typeinfo to json result ?
          if (row.createdAt) {
            row.createdAt = new Date(row.createdAt+":00");
          }
          if (row.updatedAt) {
            row.updatedAt = new Date(row.updatedAt+":00");
          }
...
//        var _query = new Query(dbs[table].definition);
        ...
        // Build Query
//        var query = new Query(dbs[table_clean].schema).update(table, options, data);
        var query = "SELECT * FROM " + table_clean + "_update($1, $2);"

        // Run Query
//        client.query(query.query, query.values, function __UPDATE__(err, result) {
        client.query(query, [options, data], function __UPDATE__(err, result) {
          if(err) return cb(err);
...
//////////////////////////////////////////////////////////////////////////////////////
    ...
    destroy: function(table, options, cb) {
      ...
      spawnConnection(function __DELETE__(client, cb) {
        ...
        // Build Query
        var query = "SELECT * FROM " + table_clean + "_destroy($1);\n";

        // Run Query
        client.query(query, [options], function __DELETE__(err, result) {
          if(err) return cb(err);
          cb(null, result.rows);
        });
