...
def build_sort(options):
    ...
    if not sort:
        return []
    sort_list = ['ORDER BY']
    if (type(sort) in (dict, OrderedDict)):
...
def build_find_query(table, options_in):
    # json.loads('{"foo":1, "bar": 2}', object_pairs_hook=collections.OrderedDict) 
    #options = json.loads(options_in)
    options = json.loads(options_in, object_pairs_hook=OrderedDict) # to keep key order like V8 objects
    query = ['SELECT',
